...
class Creator {
   ...
   */
  createRandom(width: number, height: number, density: number | null = null) {
    const start = new Date().getTime();
    if (density === null) {
      density = Utility.getRandomIntBetween(200, 800) / 1000; // between 0.2 and 0.8
    }

    let puzzleValid = false,
      densityValid = density >= 0 && density <= 1,
      cellsFilled,
      chanceOfCellFill = 0,
      solutionGrid,
      rowArray,
    ...
    while (puzzleValid === false) {
      chanceOfCellFill = densityValid
        ? density
        : Utility.getRandomIntBetween(200, 800) / 1000;
      solutionGrid = [];
      rowArray = [];
      cellsFilled = 0;
      ...
      if (solver.solve()) {
        ...
        this._log(
          "Puzzle is solvable - solved in " + solver.elapsedTime + " seconds",
        );
        this._logLine();
   ...
   */
  createFromGrid(grid: Array<Array<number>>) {
   ...
   */
  private static _populatePuzzleFromGrid(
    puzzle: Puzzle,
