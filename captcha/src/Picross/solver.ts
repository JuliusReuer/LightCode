...
class Solver {
  ...
  solve(): boolean {
    ...
    while (
      this._getProgress() > lastProgress &&
      this._getTotalSolved() < this.puzzle.cells.length
    ) {
      ...
      this._log(
        "Pass " +
          ...
          passElapsedTime +
          " seconds :: " +
          this._getTotalSolved() +
          "/" +
          this.puzzle.cells.length +
          " cells solved",
        "info",
      );
      pass++;
    ...
    this._log(
      "Solve algorithm finished in " + totalElapsedTime + " seconds.",
      "info",
    );
  ...
  eliminateImpossibleFits(line: PuzzleLine) {
    ...
    for (
      lineSectionKey = 0;
      lineSectionKey < line.sections.length;
      lineSectionKey++
    ) {
      section = line.sections[lineSectionKey];
      newPossibleStartIndexes = Utility.cloneArray(
        section.possibleStartIndexes,
      );
      ...
      for (
        startIndexKey = 0;
        startIndexKey < section.possibleStartIndexes.length;
        startIndexKey++
      ) {
        ...
        if (
          possibleStartIndex < minimumStartIndex ||
          possibleStartIndex > maximumStartIndex
        ) {
          newPossibleStartIndexes = Utility.removeFromArray(
            newPossibleStartIndexes,
            possibleStartIndex,
          );
        }
  ...
  findKnownPositivesAndNegatives(line: PuzzleLine) {
    ...
    for (sectionKey = 0; sectionKey < line.sections.length; sectionKey++) {
      ...
      for (cellCountKey = 0; cellCountKey < cellCounts.length; cellCountKey++) {
        ...
        if (
          ...
          cellCount === section.possibleStartIndexes.length
        ) {
    ...
    for (
      cellCountKey = 0;
      cellCountKey < totalCellCounts.length;
      cellCountKey++
    ) {
    ...
    for (chainKey = 0; chainKey < chains.length; chainKey++) {
      ...
      if (chain.length === firstSortedSection.length) {
        ...
        if (line.cells[chain.start + firstSortedSection.length]) {
          this._setCellSolution(
            line.cells[chain.start + firstSortedSection.length],
            0,
          );
        }
  ...
  findCompletedSections(line: PuzzleLine) {
    ...
    for (sectionKey = 0; sectionKey < line.sections.length; sectionKey++) {
      ...
      if (!section.solved && section.possibleStartIndexes.length === 1) {
        ...
        if (
          line.cells[firstNegative] &&
          line.cells[firstNegative].aiSolution === null
        ) {
          this._setCellSolution(line.cells[firstNegative], 0);
        }
        if (
          line.cells[lastNegative] &&
          line.cells[lastNegative].aiSolution === null
        ) {
  ...
  private _reset() {
    ...
    for (
      columnKey = 0;
      columnKey < this.puzzle.columnHints.length;
      columnKey++
    ) {
  ...
  private _setCellSolution(puzzleCell: PuzzleCell, value: number) {
  ...
  private _log(html: any, cssClass: any) {
  ...
  private _getProgress(): number {
    ...
    for (lineKey = 0; lineKey < this.lines.length; lineKey++) {
      ...
      maxPossibilities +=
        ...
        (line.type === "row" ? this.puzzle.width : this.puzzle.height);

      for (sectionKey = 0; sectionKey < line.sections.length; sectionKey++) {
        totalPossibilities +=
          line.sections[sectionKey].possibleStartIndexes.length;
      }
    }
